...
async function fetchTokenData(ca: string) {
  try {
    ...
    return {
      ...
      imageUrl: pair.info?.imageUrl || pair.info?.header || pair.imageUrl || pair.info?.icon || "",
  ...
  app.post("/api/ai/analyse", async (req, res) => {
    ...
    try {
      ...
      const marketData = {
        ...
        fdv: dexPair.marketCap || dexPair.fdv || 0,
      ...
      if (!finalMarket && typeof marketData !== 'undefined') {
        finalMarket = await storage.createMarket({
          ...
          ca: ca,
          imageUrl: dexPair.info?.imageUrl || dexPair.info?.header || dexPair.imageUrl || dexPair.info?.icon || "",
